...
export default function App() {
  ...
  const enroll = async () => {
    ...
    setScreen('status');
    startHeartbeat(orgToken.trim());
  };

  const startHeartbeat = (token) => {
    const send = async () => {
      try {
        const res = await fetch(`${SERVER}/api/android/heartbeat`, {
          method: 'POST',
  ...
  if (screen === 'enroll') return (
    <View style={styles.container}>
      ...
      <TextInput
        style={styles.input}
        placeholder="Paste org token here"
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.sub}>Device is enrolled and reporting</Text>
    </View>
  );
}
